...
- [x] **Contact form backend.** Connected to Web3Forms (client-side API via `fetch`, no server needed — fits GitHub Pages). Submissions currently go to `greenloaning@proton.me` (a testing address) — before real launch, generate a fresh access key at web3forms.com against the real business inbox (e.g. `adminteam@greenloaning.com.au`) and swap the `access_key` value in `contact.html`.
